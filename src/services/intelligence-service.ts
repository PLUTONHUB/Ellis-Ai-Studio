...
function inferIndustry(services: string[], text: string) { if (/roof|plumb|hvac|electric/.test(`${services.join(" ")} ${text}`.toLowerCase())) return "Home services"; if (/legal|attorney/.test(text)) return "Legal services"; if (/dental|medical|clinic/.test(text)) return "Healthcare"; return services[0] ? "Professional services" : "Not confidently identified"; }
