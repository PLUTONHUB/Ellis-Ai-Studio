...
import type { GoogleBusinessDashboard } from "~/lib/google-business.server";

const loadDashboard = createServerFn({ method: "GET" }).handler(async () => {
  const { dashboard } = await import("~/lib/google-business.server");
  return dashboard();
});
const unlock = createServerFn({ method: "POST" }).validator((data: { accessToken: string }) => data).handler(async ({ data }) => {
  const { unlockDashboard } = await import("~/lib/google-business.server");
  return unlockDashboard(data.accessToken);
});
const beginOauth = createServerFn({ method: "POST" }).handler(async () => {
  const { authorizationUrl } = await import("~/lib/google-business.server");
...
function ConnectedProfiles({ data }: { data: GoogleBusinessDashboard }) {
