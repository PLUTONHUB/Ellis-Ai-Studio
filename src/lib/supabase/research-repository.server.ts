...
export class SupabaseResearchRepository {
  ...
  async saveClientIntake(input: { companyName: string; ownerName: string; email: string; phone?: string; address?: string; websiteUrl?: string; googleBusinessProfileUrl?: string; serviceAreas: string[]; servicesOffered: string[]; goals: string[]; currentSoftware: string[]; preferredIntegrations: string[]; status: "submitted" | "research_started" | "research_running" | "research_complete" | "research_failed"; researchStatus: "not_started" | "started" | "running" | "complete" | "failed"; researchRunId?: string; researchError?: string }): Promise<void> {
    const { error } = await this.client.from("client_intakes").upsert({ company_name: input.companyName, owner_name: input.ownerName, email: input.email, phone: input.phone ?? null, address: input.address ?? null, website_url: input.websiteUrl ?? null, google_business_profile_url: input.googleBusinessProfileUrl ?? null, service_areas: input.serviceAreas, services_offered: input.servicesOffered, goals: input.goals, current_software: input.currentSoftware, preferred_integrations: input.preferredIntegrations, onboarding_status: input.status, research_status: input.researchStatus, research_run_id: input.researchRunId ?? null, research_error: input.researchError ?? null }, { onConflict: "email,company_name" });
    if (error) throw error;
  }
